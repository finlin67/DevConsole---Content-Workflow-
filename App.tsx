...
export default function DevConsole() {
  ...
  const handleSync = async () => {
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        contents: 'Provide a extremely short 5-8 word technical system status update for a content marketing workflow console. Example: ALL_NODES_OPTIMIZED_CONVERSION_PEAK_DETECTED. No punctuation.',
      });
      const insight = response.text?.trim().toUpperCase() || 'INSIGHT_FETCH_TIMEOUT';
      addLog(insight, 'ai');
    } catch (e) {
  ...
  return (
    <div className="w-full h-full flex justify-center items-center bg-[#0f172a]">
      <div className="w-[600px] h-[600px] overflow-hidden relative bg-[#070b14] text-white selection:bg-cyan-500/40 flex flex-col p-5 font-mono border-[12px] border-slate-900 shadow-[inset_0_0_100px_rgba(0,0,0,0.8),0_20px_50px_rgba(0,0,0,1)]">
